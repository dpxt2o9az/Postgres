 ...
 * @author Brad
 */
public class OracleDialect extends TreeMap<String, String> {

    {
        this.put("INSERT_COUNTRY_CODES", "insert into country_codes ( country_code, description ) values ( ?, ? )");
        this.put("FETCH_COUNTRY_CODES", "select * from country_codes");
        this.put("INSERT_AOI_COUNTRY_CODES", "insert into aoi_country_codes ( aoi_code, country_code ) values ( ?, ? )");
        this.put("INSERT_AOI_CODES", "insert into aoi_codes ( aoi_code, description ) values ( ?, ? )");
